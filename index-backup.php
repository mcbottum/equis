...
$appt=$_SESSION['appt'];
$active_div=$_REQUEST['active_div'];
if(!$active_div){
	$active_div='A';
}
//print $active_div;
...
//set and get initial values for site
	...
	$filters=mysql_query($filter_sql,$conn) or die(mysql_error());
	...
	$physicals=getReminders("physical",1,"equis" ,$_SESSION[facility]);
	//$vaccinations=getReminders("vaccination",1,"equis" ,$_SESSION[facility]);
	$dental=getReminders("dental",1,"equis" ,$_SESSION[facility]);
	$farrier=getReminders("farrier",1,"equis" ,$_SESSION[facility]);
	$role_results=getRoles($_SESSION['facility']);
...
<?
if($active_div==A){
     print "<div id='fragment-A' class='active'>";//make this div active or not
}else{
	 print "<div id='fragment-A'>";
}
if($horse_key&&$_SESSION['passwordcheck']=='pass'&&$role_mode==0){//horse_key mode
print "<div class='accordion'>";
      if($_SESSION['access'] < 4){
        	...
        	foreach($infoResult as $row){
  	  	 		...
  	  	 		print "<div id='delete_info".$row[key]."' class='inner_accordion'>";
 ...
 <form action="image_upload.php" method="post" enctype="multipart/form-data">
 		 	...
 		 	print "No Information has been recorded for ". $horse_name;
 		 }//end result info if
        }//end access if
...
elseif(isset($role_key)&&$role_mode==1&&$_SESSION['passwordcheck']=='pass'&&$access<3){//put caval-connect in data entry mode only
	...
	print "<input type='hidden' name='nhorse_record' id='urole_horse_key".$horse[key]."' value='".$horse[key]."' />";
		...
		print "<br></br>";
        print "<div align='center'><IMG id='home_image' STYLE='border:2px solid black; border-radius:5px; width:300px; horizontal-align:center' SRC='img/cover.jpg' ALT='Home'></div>";
        print "<div align='center'><IMG id='logo' STYLE=' width:500px;padding-top:-10px;' SRC='img/caval-connect_logo.png' ALT='Home'> </div>";
...
<?
if($active_div==1){
     print "<div id='fragment-1' class='active'>";//make this div active or not
}else{
  	  	 		...
  	  	 		foreach($vaccination_field_labels as $label){
  	  	 			if($label!="key"&&$label!="horse_key"&&$label!="vet_key"&&$label!="facility_key"&&$label!="date"&&$label!="next_date"){
  	  	 				...
  	  	 				if($row[$label]!=0){
  	  	 					...
  	  	 					print "<tr class='bottom_borderr'><td class='bold' width='150'>vet</td><td>Owner Administered</td></tr>";
  	  	 				}
  	  	 			}elseif($label=="date"){
  	  	 				...
  	  	 				print "<tr>";
								...
								print "</td></tr>";
  	  	 					...
  	  	 					print "<td>";
									...
									print "<option value='owner'>Owner administered</option>";
									print "<option value='0'>New Vet</option>";
								print "</select>";
  	  	 					print "</td>";
  	  	 				print "</tr>";
  	  	 				...
  	  	 				print "<tr>";
								...
								<?
  	  	 					...
  	  	 					print "<td>";
									...
									print "<option value=''> Select Vet or Farrier </option>";
										foreach($vet_data as $data){
											print"<option value=$data[key]>$data[first_name] $data[last_name]</option>";
										}//end foreach
										foreach($farrier_data as $data){
									...
									print "<option value='0'>New Vet/Farrier</select>";
								print "</select>";
  	  	 					print "</td>";
  	  	 				print "</tr>";
			print "</table>";
...
<?
	if($_SESSION['passwordcheck']=='pass'){
        if($login_data){
        	...
        	print "<div id='admini'>";//inner accordion
        	print "<table>";
        	  	 print "<tr>";
